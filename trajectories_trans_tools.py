...
def compute_speeds(a):
	...
	'''
	ret = a[:,1:,:]-a[:,:-1,:] 
	ret = torch.cat([en_cuda(torch.zeros(a.size()[0],1,a.size()[2])),ret],1)
	return ret
...
def create_rotation_matrices(angles,real = False):
	...
	for i in range(angles.size()[0]):
		...
		matrices.append([[np.cos(angle),-np.sin(angle)],[np.sin(angle),np.cos(angle)]])
...
def generate_tracklets(scene,factor = 10,compute_neighbors=True,neighboring_dist = 6):
	...
	# Group by frame, store groups in "grp_by_frames" and map frame number to location in "grp_by_frames" using "unique_ped_in_frame"
	unique_ped_in_frame = {}
	idx = 0
	...
	final_tracklets = []
	# Iterate over all pedestrians positions and find their neighbors if compute_neighbors is True
	for x in grp_by_ped_id:
		...
		if compute_neighbors:
			# Find x neighbors
			for frame_id in x[0]:
				neighb = grp_by_frames[frame_pos_mappings[frame_id]][1].values.tolist()
			...
			for neighb_id in set_neighb:
				...
				select = neighb[(neighb[0] >= min_frame) & (neighb[0] <= max_frame)].as_matrix()
				...
				for neighb_pos in select:
					if idx_frame is None:
						...
						idx_frame += 1
					distance = (neighb_pos[2]-x.iloc[idx_frame,2])**2 + (neighb_pos[3]-x.iloc[idx_frame,3])**2
			...
			spec['mean_nb_critical_neighbors_per_frame'] = functools.reduce(lambda x,y : x+y,mean_nb_critical_neighbors)/20.0
			spec['mean_contact'] = mean_contact/float(nb_critical_neighbors) if nb_critical_neighbors > 0 else 0
			spec['ped_id_grp_pos_mapping'] = ped_id_grp_pos_mapping
			final_tracklets.append((en_cuda(torch.Tensor(x.as_matrix())),buffer_neighbors,spec))
		else :
			final_tracklets.append((en_cuda(torch.Tensor(x.as_matrix())),buffer_neighbors,{}))
...
def transform_tracklets_trajectories(trajs,compute_neighbors=True):
	...
	for traj in trajs:
		...
		angle = np.arctan2(traj[0][1,3]-center_point[1],traj[0][1,2]-center_point[0])
		rotation_matrix = create_rotation_matrices(angle,real = True)
		rotated_pos_c = rotate(rotation_matrix,traj[0][:,[2,3]].sub(center_point).unsqueeze(0)).squeeze(0)
		rotated_pos_c = torch.cat([traj[0][:,[0,1]],rotated_pos_c],1)
		buffer_neighb = []
		if compute_neighbors:
			for neighb in traj[1]:
				rotated_pos = rotate(rotation_matrix,neighb[:,[2,3]].sub(center_point).unsqueeze(0)).squeeze(0)
