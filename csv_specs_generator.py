...
# Getting datasets specs
path = 'FOLDER CONTAINING THE DATASETS'
idx = 0
	...
	if( not dataset.startswith('.') and dataset not in ['validation']):
		...
		for file in os.listdir(path+'/'+dataset):
			if( not file.startswith('.')):
				...
				# For every transformed tracklet append the corresponding infos to the dataframe to Save (generate_tracklets doc for more infos)
				for result in transform_tracklets_trajectories(res,compute_neighbors=False):
					rotated.append(result[0].unsqueeze(0)) 
					nb_critical_neigbors.append(result[2]['nb_critical_neighb'])
					nb_neigbors.append(len(result[2]['neighb']))
					mean_nb_neighbors.append(result[2]['mean_nb_neighbors_per_frame'])
					mean_nb_critical_neighbors.append(result[2]['mean_nb_critical_neighbors_per_frame'])
				...
				# tracklet specs
				df_tmp = pd.DataFrame(columns = ['Dataset','File','Track_ID','Mean_rotation','nb_neighbors','nb_critical_neighbors','mean_nb_neighbors_per_frame','mean_nb_critical_neighbors_per_frame','mean_contact','is_static'])
				...
				df_tmp['mean_nb_critical_neighbors_per_frame']  = mean_nb_critical_neighbors
				df_infos.append(df_tmp)
# Save final Dataframe
df = pd.concat(df_infos).reset_index().drop(['index'],axis = 1)
df.to_csv('stats_datasets.csv')
...
def compute_relative_angle(trajectories):
	...
	for idx,traj in enumerate(speeds):
		...
		if (len(cond[idx,:].nonzero().size()) == 0):
			...
			trajectories_without_stops.append(traj[cond[idx,:].nonzero().squeeze(1),:])
	...
	for idx in range(len(trajectories_without_stops)):
		angles = torch.abs(torch.atan2(trajectories_without_stops[idx][:,1], trajectories_without_stops[idx][:,0])-(0.5*np.pi))
